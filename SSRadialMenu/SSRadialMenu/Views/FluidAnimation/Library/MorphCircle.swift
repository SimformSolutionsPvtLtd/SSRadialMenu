...
struct MorphCircle: Shape {
    ...
    init(isDragging: Bool, isMenu: Bool) {
        self.angleIntervals = Array(stride(from: CGFloat(0.0), to: 360.0, by:  30))
    ...
    func path(in rect: CGRect) -> Path {
        ...
        var dragCirclePoints = circlePoints
            .map { $0.scaled(by: Double(isMenu ? radius + 10 : radius + 40)) }
            .map { $0 + center }
        
        if !isMenu {
            print("centre:", center)
            print("dragCirclePoints", dragCirclePoints[0])
            print("point", points[0])
           
        }
        points[0] = lerp(points[0], dragCirclePoints[0], by: self.animatableData)
...
//            if isDragging {
//                controlPoint1 = CGPoint(x: controlPoint1.x/2, y: controlPoint1.y/2)
//                controlPoint2 = CGPoint(x: controlPoint2.x/2, y: controlPoint2.y/2)]
//            }
...
//            if index == 6 {
//                print("prevpoint:", point0)
//                print("currpoint:", point1)
//                print("nextpoint:", point2)
