...
struct RadialMenu: View {
    ...
    private func createMenuItem(_ item: MenuItem, at index: Int, position: Position) -> some View {
        ...
        return ZStack {
            BlurredOverlayCircles(
                xOffset: menuItemsVisible[index] ? $xOffset : .constant(0),
                yOffset: menuItemsVisible[index] ? $yOffset : .constant(0),
                scaleEffect: menuItemsVisible[index] ? $scaleEffect : .constant(0),
                color: item.color
            )
            item.menuView
        }
